...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #dy
        self.index_to_session = [label_file.split(os.sep)[-1].split('_')[0] for label_file in self.label_files]
        sessions = defaultdict(list)
        for index, session in enumerate(self.index_to_session):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < 0.8:
                img, labels, lams = load_mosaic(self, index)
            else:
                img, labels, lams = load_mosaic9(self, index)
            shapes = None
        ...
        if self.augment:
            # Augment imagespace
            if not mosaic:
                img, labels, lams = random_perspective(img, labels, lams,
                                                 degrees=hyp['degrees'],
        ...
        return torch.from_numpy(img), labels_out, lams, self.img_files[index], shapes

    @staticmethod
    def collate_fn(batch):
        img, label, lams, path, shapes = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), torch.cat(lams, 0), path, shapes
...
def load_mosaic(self, index):
    ...
    img4, labels4, lams4 = random_perspective(img4, labels4, lams4, segments4,
...
def load_mosaic9(self, index):
    ...
    img9, labels9, lams9 = random_perspective(img9, labels9, lams9, segments9,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
...
def random_perspective(img, targets=(), lams=None, segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
                       border=(0, 0)):
...
def load_crop_mix_paste(self, index, use_session=True, alpha=0.2):
    ...
    for i in range(len(base_label)):
        # 박스 단위로 확률 적용
        if random.random() < self.hyp['crop_mix']:
    ...
    for selected_box_idx, selected_box in selected_boxes:
        ...
        # bbox가 하나라도 있는 src를 선택할 때까지 반복
        while len(src_label) == 0 or len(src2_label) == 0:
            if use_session:
                ...
                src2_idx = random.choice(self.indices)
        ...
        src2_image, _, _ = load_image(self, src2_idx)
    ...
    if len(lam_list) > 0:
        lams = np.concatenate((lams, np.array(lam_list)), axis=0)
