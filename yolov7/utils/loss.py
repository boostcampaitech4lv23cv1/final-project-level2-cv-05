...
class ComputeMixedLoss:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeMixedLoss, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def __call__(self, p, targets, lams):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tlam, tbox, indices, anchors = self.build_targets(p, targets, lams)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    self.BCEcls.weight = tlam[i].view(-1, 1)
    ...
    def build_targets(self, p, targets, lams):
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)

        lams = lams.view(-1, 1)
        targets = torch.cat((targets, lams), axis=1)

        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        tcls, tlam, tbox, indices, anch = [], [], [], [], []
        ...
        for i in range(self.nl):
            ...
            anch.append(anchors[a])  # anchors
            tcls.append(c)  # class
            tlam.append(lam) # lambda

        return tcls, tlam, tbox, indices, anch


class ComputeMixedLossOTA:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeMixedLossOTA, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def __call__(self, p, targets, lams, imgs):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p, targets, lams, imgs)
        pre_gen_gains = [torch.tensor(pp.shape, device=device)[[3, 2, 3, 2]] for pp in p] 
    ...
    def build_targets(self, p, targets, lams, imgs):
